...
export class QiniuUploadService {
    ...
    public getStaticUploader(form_data: any) {
        ...
        return new Observable((observer: Observer<any>) => {
            this.http.get(`${Constants.Urls['getPubToken']}`, {headers: headers, withCredentials: true})
                .map(res => res.json())
                .subscribe(data => {
    ...
    public getDataUploader() {
        ...
        headers.append('Content-Type', 'application/x-www-form-urlencoded');
        return new Observable((observer: Observer<any>) => {
            this.http.get(`${Constants.Urls['getPriToken']}`, {headers: headers, withCredentials: true})
                .map(res => res.json())
                .subscribe(data => {
                    let options = {
                        runtimes: 'html5,flash,html4',
